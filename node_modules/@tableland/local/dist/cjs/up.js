...
const argv = (0, yargs_1.default)((0, helpers_1.hideBin)(process.argv)).options({
    validator: {
        type: "string",
        description: "Path the the Tableland Validator directory. If docker flag is set, this must be the full repository.",
    },
    registry: {
        type: "string",
...
const go = async function () {
    ...
    if (tsArgv.docker)
        opts.docker = tsArgv.docker;
    if (typeof tsArgv.verbose === "boolean")
        opts.verbose = tsArgv.verbose;
    if (typeof tsArgv.silent === "boolean")
        opts.silent = tsArgv.silent;
    if (typeof tsArgv.registryPort === "number")
        opts.registryPort = tsArgv.registryPort;
    const tableland = new main_js_1.LocalTableland(opts);
