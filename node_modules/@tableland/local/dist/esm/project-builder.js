...
export const projectBuilder = async function () {
    ...
    if (shouldCreate === choices[0]) {
        console.log(`${chalk.yellow.bold("OK, if you change your mind run this again anytime!")}
  ...
  and star us on github at ${chalk.cyan(githubLink)}`);
    ...
    if (configExists()) {
        console.log(`${chalk.red.bold("Config file already exists, nothing to do.")}`);
        return;
    }
    console.log(chalk.yellow(`Creating a config file.`));
    // Copy the example config to the new project
    writeFileSync("tableland.config.js", [
        "const config = ",
