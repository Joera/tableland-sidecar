...
import { checkWait, normalize, readNameMapping, } from "./helpers/index.js";
        ...
        try {
            ...
            switch (type) {
                case "read": {
                    ...
                    const results = await queryAll(config, sql, opts.controller);
                    return wrapResult(results, performance.now() - start);
                }
                default: {
                    return wrapResult(await __classPrivateFieldGet(this, _Statement_instances, "m", _Statement_waitExec).call(this, { type, sql, tables }, opts.controller), performance.now() - start);
                }
            }
        }
        catch (cause) {
        ...
        try {
            ...
            switch (type) {
                case "read": {
                    ...
                    const results = await queryFirst(config, sql, options.controller);
        ...
        try {
            ...
            switch (type) {
                case "read": {
                    ...
                    return await queryRaw(config, sql, opts.controller);
                }
                default: {
                    await __classPrivateFieldGet(this, _Statement_instances, "m", _Statement_waitExec).call(this, {
                        type,
...
_Statement_instances = new WeakSet(), _Statement_parseAndExtract = async function _Statement_parseAndExtract() {
    ...
    return await checkWait(this.config, await exec(this.config, params), controller);
