import { extractBaseUrl, extractSigner, normalize, readNameMapping, writeNameMapping, } from "./helpers/index.js";
...
export async function queryRaw(config, statement, signal) {
    const params = { statement, format: "table" };
    const response = await getQuery(config, params, signal)
        .then((res) => res.rows)
        .catch(catchNotFound);
    return response;
}
export async function queryAll(config, statement, signal) {
    const params = { statement, format: "objects" };
    const response = await getQuery(config, params, signal).catch(catchNotFound);
    return response;
}
export async function queryFirst(config, statement, signal) {
