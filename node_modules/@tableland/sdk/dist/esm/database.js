...
import { checkWait, extractBaseUrl, normalize, readNameMapping, validateTableName, } from "./helpers/index.js";
...
export class Database {
    ...
    // everything else a single result wrapped in an Array for backward compatability.
        ...
        try {
            ...
            const normalized = await Promise.all(statements.map(async (stmt) => await normalize(stmt.toString(), nameMap)));
            ...
            if (type === "create") {
                const receipt = await checkWait(this.config, await execCreateMany(this.config, statements.map((stmt) => stmt.toString())), controller);
            ...
            const receipt = await checkWait(this.config, await execMutateMany(this.config, runnables), controller);
            // TODO: wrapping in an Array is required for back compat, consider changing this for next major
            return [wrapResult(receipt, performance.now() - start)];
        }
        catch (cause) {
