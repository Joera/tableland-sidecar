...
export async function getTransactionReceipt(config, params, signal) {
    const receiptByTransactionHash = getFetcher(config)
        .path("/receipt/{chainId}/{transactionHash}")
        .method("get")
        .create();
    const { data } = await receiptByTransactionHash(params, signal).catch(hoistApiError);
    const transformed = transformResponse(data);
    return transformed;
}
export async function pollTransactionReceipt(config, params, controller) {
    const receiptByTransactionHash = getFetcher(config)
        .path("/receipt/{chainId}/{transactionHash}")
        .method("get")
        .create();
