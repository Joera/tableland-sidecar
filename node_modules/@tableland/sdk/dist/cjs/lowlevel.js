    ...
    switch (type) {
        case "create": {
            if (typeof config.aliases?.read === "function") {
                const currentAliases = await (0, index_js_1.readNameMapping)(config.aliases);
            ...
            if (typeof config.aliases?.write === "function") {
                ...
                await (0, index_js_1.writeNameMapping)(config.aliases, nameMap);
            }
            return wrappedTx;
        }
        /* c8 ignore next */
        ...
        case "write": {
            if (typeof config.aliases?.read === "function") {
                const nameMap = await (0, index_js_1.readNameMapping)(config.aliases);
...
async function queryRaw(config, statement, signal) {
    const params = { statement, format: "table" };
    const response = await (0, query_js_1.getQuery)(config, params, signal)
        .then((res) => res.rows)
        .catch(catchNotFound);
    return response;
}
exports.queryRaw = queryRaw;
async function queryAll(config, statement, signal) {
    const params = { statement, format: "objects" };
    const response = await (0, query_js_1.getQuery)(config, params, signal).catch(catchNotFound);
    return response;
}
exports.queryAll = queryAll;
async function queryFirst(config, statement, signal) {
    const response = await queryAll(config, statement, signal).catch(catchNotFound);
