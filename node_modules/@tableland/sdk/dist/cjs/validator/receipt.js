...
async function getTransactionReceipt(config, params, signal) {
    const receiptByTransactionHash = (0, index_js_1.getFetcher)(config)
        .path("/receipt/{chainId}/{transactionHash}")
        .method("get")
        .create();
    const { data } = await receiptByTransactionHash(params, signal).catch(errors_js_1.hoistApiError);
    const transformed = transformResponse(data);
    return transformed;
}
exports.getTransactionReceipt = getTransactionReceipt;
async function pollTransactionReceipt(config, params, controller) {
    const receiptByTransactionHash = (0, index_js_1.getFetcher)(config)
        .path("/receipt/{chainId}/{transactionHash}")
        .method("get")
        .create();
    ...
    const control = controller ?? (0, chains_js_1.getChainPollingController)(params.chainId);
    const receipt = await (0, await_js_1.getAsyncPoller)(fn, control);
    return receipt;
}
