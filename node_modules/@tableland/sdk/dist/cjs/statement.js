...
var _Statement_instances, _Statement_parseAndExtract, _Statement_waitExec, _Statement_checkIsValidOpts;
...
class Statement {
        ...
        try {
            ...
            switch (type) {
                case "read": {
                    ...
                    const results = await (0, lowlevel_js_1.queryAll)(config, sql, opts.controller);
                    return (0, utils_js_1.wrapResult)(results, performance.now() - start);
                }
                default: {
                    return (0, utils_js_1.wrapResult)(await __classPrivateFieldGet(this, _Statement_instances, "m", _Statement_waitExec).call(this, { type, sql, tables }, opts.controller), performance.now() - start);
                }
            }
        }
        catch (cause) {
    ...
    async first(colNameOrOpts, opts = {}) {
        try {
            // Handle first overload to ensure passed `opts` are not set to `colName`
            const colNameIsOpts = __classPrivateFieldGet(this, _Statement_instances, "m", _Statement_checkIsValidOpts).call(this, colNameOrOpts);
            ...
            switch (type) {
                case "read": {
                    ...
                    const results = await (0, lowlevel_js_1.queryFirst)(config, sql, options.controller);
        ...
        try {
            ...
            switch (type) {
                case "read": {
                    ...
                    return await (0, lowlevel_js_1.queryRaw)(config, sql, opts.controller);
                }
                default: {
                    await __classPrivateFieldGet(this, _Statement_instances, "m", _Statement_waitExec).call(this, {
                        type,
...
_Statement_instances = new WeakSet(), _Statement_parseAndExtract = async function _Statement_parseAndExtract() {
    ...
    const nameMap = typeof this.config.aliases?.read === "function"
        ? await (0, index_js_1.readNameMapping)(this.config.aliases)
        : undefined;
...
}, _Statement_waitExec = async function _Statement_waitExec(params, controller) {
    return await (0, index_js_1.checkWait)(this.config, await (0, lowlevel_js_1.exec)(this.config, params), controller);
}, _Statement_checkIsValidOpts = function _Statement_checkIsValidOpts(opts) {
    return opts != null && (0, await_js_1.isPollingController)(opts.controller);
};
