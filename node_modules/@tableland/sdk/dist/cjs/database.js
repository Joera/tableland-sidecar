...
class Database {
    ...
    async batch(statements, controller
    ...
    // everything else a single result wrapped in an Array for backward compatability.
        ...
        try {
            ...
            const normalized = await Promise.all(statements.map(async (stmt) => await (0, index_js_1.normalize)(stmt.toString(), nameMap)));
            ...
            if (type === "read") {
                return await Promise.all(statements.map(async (stmt) => await stmt.all({ controller })));
            }
            // For "create" statement types, each statement must be a single create sql query
            if (type === "create") {
                const receipt = await (0, index_js_1.checkWait)(this.config, await (0, lowlevel_js_1.execCreateMany)(this.config, statements.map((stmt) => stmt.toString())), controller);
            ...
            const receipt = await (0, index_js_1.checkWait)(this.config, await (0, lowlevel_js_1.execMutateMany)(this.config, runnables), controller);
            // TODO: wrapping in an Array is required for back compat, consider changing this for next major
            return [(0, utils_js_1.wrapResult)(receipt, performance.now() - start)];
        }
        catch (cause) {
     ...
     */
    async exec(statementStrings, controller) {
        ...
        try {
            ...
            return (0, utils_js_1.wrapExecResult)(result, count);
        }
        catch (cause) {
...
async function normalizedToRunnables(normalized) {
    ...
    if (normalized.tables.length > 1) {
        ...
        for (const stmt of normalized.statements) {
            ...
            const norm = await (0, index_js_1.normalize)(stmt);
            if (norm.tables.length > 1) {
                ...
                const filteredTables = norm.tables.filter((tableName) => !tableNames.includes(tableName));
